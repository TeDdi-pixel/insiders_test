  ...
  return (
    ...
    >
      <div
        ...
        className={`relative w-full h-[48px] border-x border-t border-border flex items-center pl-6 cursor-pointer ${
          !active ? "border-b" : ""
        }`}
      >
        Select {placeholder}
      ...
      >
        {data.map((item) => (
          <div key={item.value} className="flex gap-3 flex-nowrap items-center">
            <input
              type="checkbox"
              id={item.value}
